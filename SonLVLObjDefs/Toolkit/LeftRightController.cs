...
{
	class LeftRightController : ObjectDefinition
	{
		...
		{
			sprites[0] = new Sprite(LevelData.GetSpriteSheet("Toolkit/SonLVL.gif").GetSection(51, 1, 32, 32), -16, -16);
		...
		public override Sprite GetSprite(ObjectEntry obj)
		{
			...
			while (i < count) {
				...
				short X = (short)(obj.X + range + (i * 24) - ((count - 1) * 12));
		...
		public override Sprite GetDebugOverlay(ObjectEntry obj)
		{
			int range = ((int)((V4ObjectEntry)obj).Scale);
			range *= 4;
			BitmapBits overlay = new BitmapBits(range * 2 + 1, 1);
			overlay.DrawLine(6, 0, 0, range * 2, 0);
			Sprite debug = new Sprite(overlay, -range, 0);
			return debug;
		}
	}
}